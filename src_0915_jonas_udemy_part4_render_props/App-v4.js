...
const HOCProductList = withToggles(ProductList);

function withToggles(WrapperComponent) {
  return function List({ title, items }) {
    console.log("title", title);
    console.log("items", items);

    const { isOpen, isCollapsed, setIsCollapsed, toggleOpen } =
      useShowVisible();
    const displayItems = isCollapsed ? items.slice(0, 3) : items;
    ...
    return (
      <div className="list-container">
        ...
        {isOpen && <WrapperComponent items={displayItems} />}
        <button onClick={() => setIsCollapsed((isCollapsed) => !isCollapsed)}>
          {isCollapsed ? `Show all ${items.length}` : "Show less"}
        </button>
      </div>
  ...
  return (
    <div>
      ...
      <div className="col-2">
        ...
        <HOCProductList title="Products HOC" items={products} />
      </div>
    </div>
  );
}
